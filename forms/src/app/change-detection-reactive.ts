...
export class ChangeDetectionReactiveComponent implements OnInit {
  ...
  atLeastOneRoleValidator<T extends FormGroupType<RegistrationForm>['controls']['roles'], E extends AtleastOneRoleValidatorError>(control: AbstractControl): ValidationErrors | null {
    const rolesControl = control as T;
    const valid = Object.keys(rolesControl.controls).some(key => rolesControl.controls[key]!.value);
    const error: E = { atLeastOneRoleValidator: true } as E;
...
interface AtleastOneRoleValidatorError {
  atLeastOneRoleValidator: true;
}
...
export type FormGroupType<T> = FormGroup<{
  [K in keyof T]: T[K] extends object ? T[K] extends Array<infer E> ? E extends object ? FormArray<FormGroupType<E>> : FormArray<FormControl<E>> : FormGroupType<T[K]> : ɵElement<T[K], never>;
}>;